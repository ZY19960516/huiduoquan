...
const app = getApp();
const tmplId = 'cmwVWp5mRbJOrwY6XS-9bPu9zzTnJlBAKdw8nCGAhR4';
...
Page({
  ...
  toCoupon(e) {
    ...
    let currentCoupon = this.data.tabs[this.data.activeTab].coupon[couponIdx]
    const type = currentCoupon.type
    const wxappinfo = currentCoupon.minapp
    const link = currentCoupon.link
    const name = currentCoupon.name
    if (type == 2) {
      wx.navigateTo({
        url: `../webView/index?url=${encodeURIComponent(link)}&title=${name}`
      })
    } else {
